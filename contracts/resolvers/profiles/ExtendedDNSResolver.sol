...
/**
 * @dev Resolves names on PNS by interpreting record data stored in a DNS TXT record.
 ...
 *          PNS1 <address or name of ExtendedDNSResolver> <record data>
 *
 *      For example:
 *          PNS1 2.dnsname.pns.pls a[1028]=0x1234...
 *
 ...
 *         `coinType`. PulseChain has `coinType` 1028. The value must be 0x-prefixed hexadecimal, and will
 ...
 *         Examples:
 *          - a[1028]=0xFe89cc7aBB2C4183683ab71653C4cdc9B02D44b7
 ...
 *           https://github.com/pulsedomains/address-encoder/blob/master/docs/supported-cryptocurrencies.md
 *         Example:
...
contract ExtendedDNSResolver is IExtendedDNSResolver, IERC165 {
    ...
    function _resolveAddress(
        ...
        bytes memory value;
        // Per https://docs.pulse.domains/pnsip/11#specification
    ...
    function _resolveAddr(
        ...
        bytes memory value = _findValue(context, "a[1028]=");
        if (value.length == 0) {
            return value;
        }
        (bytes memory record, bool valid) = value.hexToBytes(2, value.length);
