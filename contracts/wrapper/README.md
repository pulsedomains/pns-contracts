...
The NameWrapper wraps PNS names, adding new functionality to them:

- Makes all PNS names at any level into ERC1155 tokens.
...
- .pls names can be registered and renewed via the wrapper, removing any need to wrap names separately after registration.
- Owners of names can create wrapped subdomains directly, without having to register-then-wrap.

## Glossary of terms

- Wrap - Transfers ownership of the PNS name to the Name Wrapper. The wrapper then issues an ERC1155 token for the name.
...
- Expiry - Expiry is the date when the name expires and is no longer able to be owned. Expiry only comes into effect when a name is emancipated or locked. Expiry can be extended by parent or approved addresses, and if the parent burnt the parent controlled fuses(PCF), then the subnames can extend their expiry by themselves. However, the PCF can be unburnt later if the parent name expires. For the latest update, refer to: https://discuss.pulse.domains/t/namewrapper-updates-including-testnet-deployment-addresses/14505/48
...
Unwrapped names do not expire, with the exception of .pls second-level names, which have behaviour enforced by the .pls registrar.
...
Wrapped names do not expire, with the exception of .pls second-level names, which have behaviour enforced by the .pls registrar, and have a wrapper expiry equal to the end of the name's grace period.

To check if a name is Wrapped, verify that `NameWrapper.ownerOf(node)` does not return `address(0)`, `Registry.owner` is the NameWrapper contract and if it's a .pls name `registrar.ownerOf(labelhash)` must be the NameWrapper contract. If any of these are false, the name should be consider unwrapped.
...
.pls second-level names are automatically Emancipated when wrapped, and their expiry is fixed at the end of the grace period in the .pls registrar.
...
`setChildFuses()` and `renew()` do not have any direct restrictions around when they can be called to extend expiry. `renew()` cannot be called on a name that has expired (past grace period) on the .pls registrar and must be re-registered instead.

`setSubnodeOwner()` and `setSubnodeRecord()` both revert when the subdomain is Emancipated or Locked.

`renew()` indirectly extends the expiry of a .pls name by renewing the name inside the .pls registrar.
...
If this fuse is burned, a name will be able to extend its own expiry in the NameWrapper. Does not apply to .pls 2LDs, as the expiry will inherit from the registrar in that case, and this fuse will not be burned when wrapping/registering .pls 2LDs.
...
Wraps any name other than a .pls second-level name.
...
Allows a registrar controller to renew a .pls second-level name.
...
Unwraps a .pls second-level name, provided that the name is not Locked.
...
Unwraps any name other than a .pls second-level name, provided that the name is not Locked.
...
When called with an owner of `address(0)`, unwraps the name provided it is not Locked and not a .pls second-level name.
...
The NameWrapper mirrors most of the functionality from the original PNS registry in its own API.
...
`setRecord()` acts differently depending if the name is .pls or not. If you set the owner to `address(0)`, it will automatically unwrap the name and set the registry owner to 0, deleting the subdomain. If you set the owner to `address(0)`, but the name is a .pls 2LD, the function will revert as the ERC721 token cannot be set to the zero address. If the owner is not 0, it will simply transfer the ERC1155 to the specified owner within NameWrapper and set the resolver and ttl on the registry.
...
Since the NameWrapper does have `onERC721Received` support, the NameWrapper WILL recognise a transfer of the .pls registrar ERC721 token to the NameWrapper as a wrap.
...
Setting metadata service to https://metadata.pulse.domains/name/0x{id}
...
verify  0x08f2D8D8240fC70FD777358b0c63e539714DD473 with arguments https://metadata.pulse.domains/name/0x{id}
...
2. Set the label (`matoken` for `matoken.pls`) to `SEED_NAME=` on `.env`
3. Run `yarn seed:rinkeby`

```
~/.../pns/name-wrapper (seed)$yarn seed:rinkeby
yarn run v1.22.10
...
Wrapped NFT for wrappertest4.pls is available at https://testnets.opensea.io/assets/0x88ce50eFeA21996B20838d5E71994191562758f9/42538507198368349158588132934279877358592939677496199760991827793914037599925
Wrapped NFT for sub2.wrappertest4.pls is available at https://testnets.opensea.io/assets/0x88ce50eFeA21996B20838d5E71994191562758f9/22588238952906792220944282072078294622689934598844133294480594786812258911617
```
