...
async function setTLDsOnRoot(
  ...
  for (const tld of tlds) {
    ...
    const ownerOf = await registry.owner(ethers.utils.namehash(tld))
    if (registrar.address !== ownerOf) {
      console.log(`Transferring .${tld} to new DNS registrar`)
      const tx = await root.setSubnodeOwner(labelhash, registrar.address, {
        from: owner,
        gasLimit: 10000000,
      })
      await tx.wait()
